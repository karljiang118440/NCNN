...
int test_video()
{
    ...
    ncnn::Net detector;  
    detector.load_param("data/model/yoloface-500k.param");
    detector.load_model("data/model/yoloface-500k.bin");
    int detector_size_width  = 320;
    int detector_size_height = 256;

    //定义106关键点预测器
    ncnn::Net landmark;  
    landmark.load_param("data/model/landmark106.param");
    landmark.load_model("data/model/landmark106.bin");
...
int test_pic()
{
   ...
   // cv::VideoCapture cap(0);
        ...
        frame = cv::imread("/home/root/airunner/data/airunner/FaceRecog/images/4.jpg");
        double start = ncnn::get_current_time();
        demo(frame, detector, detector_size_width, detector_size_height, landmark, landmark_size_width, landmark_size_height);
