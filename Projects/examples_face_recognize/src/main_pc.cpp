...
int TestRecognize_cam(int argc, char* argv[]) {
	...
	face_engine.Detect(img_src, &faces1);


	cv::Mat face1 = img_src(faces1[0].face_).clone();
	// cv::Mat face2 = img_src(faces[1].face_).clone();
	...
	cv::Mat face2 = img_src(faces[0].face_).clone();
	face_engine.ExtractFeature(face2, &feature2);
	float sim = CalculSimilarity(feature1, feature2);
	double end2 = static_cast<double>(cv::getTickCount());
...
int TestLandmark_Recognize_cam(int argc, char* argv[]) {
	...
	// face_engine.ExtractFeature(frame, &feature2);
	// float sim = CalculSimilarity(feature1, feature2);
	// double end2 = static_cast<double>(cv::getTickCount());
	...
	double time_cost3 = (end3 - start3) / cv::getTickFrequency() * 1000;
	std::cout << "time3 cost: " << time_cost3 << "ms" << std::endl;
...
int main(int argc, char* argv[]) {
	...
	return TestRecognize_cam(argc, argv);
	// return TestLandmark_cam(argc, argv);
	// return TestLandmark_Recognize_video(argc, argv);
